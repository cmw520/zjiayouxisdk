...
function huosdk_reset(){
	window:huosdk.resetToken();
}
huosdk_reset();
</script>
EOT;
    ...
    protected function huoVerify() {
        ...
        $_rq_class = new \huosdk\response\Rsaauth(false, 0, $_pri_path);
        $_auth_key = $_rq_class->getAuthkey($_key);
        $this->auth_key = $_auth_key;
        if (false == $_auth_key) {
            return hs_huosdk_responce('1001', '非法请求');
        }
        $_rq_data = $_rq_class->getRqdata($_auth_key, $_data);
        if (false == $_rq_data) {
            return hs_huosdk_responce('400', '请求数据非法');
        }
    ...
    public function isUserLogin() {
        ...
        if (empty($_mem_id)) {
            return hs_huosdk_responce('1002', '登陆已过期, 重新登录');
        }
    ...
    /**
     * @param string $huosdk_float_id 浮点ID
     */
    protected function isUserwapLogin($huosdk_float_id = '') {
        if (empty($huosdk_float_id)) {
            $_huosdk_float_id = Cookie::get('huosdk_float_id');
        }
        if (empty($_huosdk_float_id)) {
            $this->resetToken();
        }
        $_ss_class = new \huosdk\common\Authcode();
        $_str = $_ss_class->discuzAuthcode($_huosdk_float_id, 'DECODE', Config::get('config.COOKIEKEY'));
        if (empty($_str)) {
            $this->resetToken();
        }
