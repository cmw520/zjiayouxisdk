...
class DataController extends MainbaseController {
...
//        $model=M("pay");
//        $records=$model
//                ->field("p.*,g.name as gamename,m.username as username")
//                ->alias('p')
//                ->join("LEFT JOIN ".C('DB_PREFIX')."game g ON g.id=p.app_id")
//                ->join("LEFT JOIN ".C('DB_PREFIX')."members m ON m.id=p.mem_id")
//                ->where("(p.agent_id = $agent_id) AND (p.create_time > $three_month_ago)")
//                ->limit(10)
//                ->select();
//        
//        $num_of_records=count($records);
//        $this->assign("num_of_records",$num_of_records);
//        $this->assign("records",$records);
//        $this->display();
        $agent_id = $_SESSION['agent_id'];
        $three_month_ago = time() - 90 * 24 * 3600;
...
//            $where["gc.app_id"]=$app_id;    
    ...
    public function get_subids_txt() {
        $subids = $this->Huosdk_agent->getMySubAgentsIds();
        if (!$subids) {
            return false;
        }
...
//        $where="m.agent_id=$this->agid OR m.agent_id IN ($subids_txt)";
    ...
    public function normalChargeRecord() {
        $ids = $this->Huosdk_agent->getMySubAgentsIds();
        $ids[] = $this->agid;
        $ids_txt = join(",", $ids);
        $where = "p.agent_id IN ($ids_txt) ";
