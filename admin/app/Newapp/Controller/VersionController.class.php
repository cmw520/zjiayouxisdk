...
class VersionController extends AdminbaseController {
    ...
    public function edit() {
        ...
        $gv_data['logo'] = M("game")->where(array("id" => C("APP_APPID")))->getField("icon");
        $this->assign($gv_data);
        $this->display();
    }
    ...
    public function add_post() {
        if (IS_POST) {
            ...
            // 获取游戏的clientid
//            $_gc_map['app_id'] = $ver_data['app_id'];
//            $_gc_map['gv_id'] = $gv_id;
//            $gc_data = $this->gc_model->where($_gc_map)->find();
//            if ($gc_data) {
...
//            } else {
            ...
            $gc_id = M('game_client')->add($gc_data);
...
//            }
    ...
    public function edit_post() {
        if (IS_POST) {
            ...
            M("game")->where(array("id" => C("APP_APPID")))->save($game_data);
            if (false == $rs) {
                $this->error("服务器内部错误");
            } else {
    ...
    protected function get_app_down_url($verid) {
        ...
        $filepath = DIRECTORY_SEPARATOR.'sdkgame'.DIRECTORY_SEPARATOR.$initial.DIRECTORY_SEPARATOR.$verid
                    .DIRECTORY_SEPARATOR.$initial.'.apk';
