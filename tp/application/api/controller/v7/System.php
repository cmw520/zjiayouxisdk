...
class System extends Basehuo {
    ...
    public function init() {
        ...
        $_rdata['up_info'] = $this->readUpinfo($_request_data['app_id'], $_request_data['client_id']);
        $_rdata['help'] = $this->readHelpinfo();
        $_rdata['newmsg'] = 1;
        $_mem_id = Session::get('id', 'user');
        if (!empty($_mem_id)) {
            $_msg_class = new \huosdk\player\Memmsg($_mem_id);
...
//        $_rdata['splash'] = $this->readSplash();
    ...
    public function help() {
        ...
        $this->getParams($_key_arr);
        $_rdata['help'] = $this->readHelpinfo();
        if (empty($_rdata['help'])) {
            return hs_player_responce(400, '无客服信息');
        }

        return hs_player_responce(200, '获取客服信息成功', $_rdata);
    }
    ...
    public function checkDevice(){
    	...
    	$this->getParams($_key_arr);
    	$_rdata = $this->getDevinfo($this->rq_data['series']);

    	if(empty($_rdata)){
    		return  hs_player_responce(400, '设备验证失败');
    	}
    	
    	return  hs_player_responce(200, '设备验证成功', $_rdata);
    }
    ...
    private function readNewtoken() {
        $_ss_class = new \huosdk\common\Simplesec();
...
//        $_rdata = $_s_class->getSplash();
     ...
     */
    private function readUpinfo($app_id, $client_id) {
        $_g_class = new \huosdk\game\Game($app_id, $client_id);
        $_upinfo = $_g_class->getUpinfo($client_id);
        if (false == $_upinfo) {
            $_upinfo['up_status'] = 0;
            $_upinfo['url'] = '';
            $_upinfo['content'] = '';
     ...
     * array()
     */
    private function getDevinfo($series){
    	$_g_class = new \huosdk\model\Model();
    	$devinfo = $_g_class->check($series);
    	return  $devinfo;
    }
}