...
class GameController extends AdminbaseController {
    ...
    public function gList($status = 2) {
        $this->_game(true, '', 2, null, null, 4);
        $where_ands = array();
        array_push($where_ands, "g.is_delete  = ".$status);
        array_push($where_ands, "g.classify  = 4");
        $fields = array(
            'gstatus'   => array(
                "field"    => "g.is_app",
                "operator" => "="
        ...
        if (empty($_GET['gtype'])) {
            ...
            $items = $this->game_model
                ->field("g.*,gr.app_id,gi.mobile_icon m_icon")
                ->alias("g")
                ->where($where)
                ->join("left join ".C('DB_PREFIX')."game_recmd gr ON g.id = gr.app_id")
                ->join("LEFT JOIN ".C('DB_PREFIX')."game_info gi ON gi.app_id=g.game_id")
                ->limit($page->firstRow.','.$page->listRows)
                ->order('g.listorder desc , g.id desc')
                ->select();
        } else {
            $count = $this->game_model->alias("g")->join(
                "left join ".C('DB_PREFIX')."game_gt gt ON g.id = gt.app_id"
            ...
            $items = $this->game_model
                ->field("g.*,gr.app_id,gi.mobile_icon m_icon")
                ->alias("g")
                ->join("left join ".C('DB_PREFIX')."game_gt gt ON g.id = gt.app_id")
                ->join("left join ".C('DB_PREFIX')."game_recmd gr ON g.id = gr.app_id")
                ->join("LEFT JOIN ".C('DB_PREFIX')."game_info gi ON gi.app_id=g.game_id")
        ...
        $this->getRecommentStatus($items);
        $this->formatIsNew($items);
    ...
    public function formatIsNew(&$items) {
        $data = array("1" => "否", "2" => "是");
        foreach ($items as $key => $value) {
            $items[$key]['is_new_txt'] = $data[$value['is_new']];
        }
    }
    ...
    public function recommend() {
        if (isset($_POST['ids'])) {
            ...
            foreach ($app_ids as $k => $v) {
                $prev = M('game_recmd')->where(array("app_id" => $v))->find();
                if (!$prev) {
                    $data = array();
                    $data['app_id'] = $v;
    ...
    public function unrecommend() {
        if (isset($_POST['ids'])) {
            $app_ids = $_POST['ids'];
            foreach ($app_ids as $k => $v) {
                M('game_recmd')->where(array("app_id" => $v))->delete();
            }
            $this->success("设置非推荐成功！");
        }
    }

    public function getRecommentStatus(&$items) {
        foreach ($items as $key => $value) {
            if (false == strpos($value['icon'], 'upload')) {
                $items[$key]['mobile_icon'] = '/upload/'.$value['icon'];
            }
            $t = M('game_recmd')->where(array("app_id" => $value['id']))->find();
            if ($t) {
                $items[$key]['recommend_txt'] = "是";
            } else {
                $items[$key]['recommend_txt'] = "否";
            }
        }
    }
    ...
    function add_post() {
        if (IS_POST) {
            ...
            $info_data['yiosurl'] = I('post.yiosurl/s');  //安卓版下载地址 
...
//             $game_data['status'] = I('post.gstatus/d',1);  //当前状态
            ...
            $game_data['run_time'] = $game_data['create_time'];
...
//             }
            ...
            if ($this->game_model->create($game_data)) {
                ...
                if ($app_id > 0) {
                    ...
                    $gc_data['client_key'] = md5(
                        $version.md5($game_data['app_key'].$game_data['initial'].rand(10, 1000))
                    );
                    $gc_data['gv_id'] = $gv_id;
...
//                     $info_data['mobile_icon'] = $game_data['icon'];
    ...
    private function _getTree() {
        ...
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        foreach ($result as $r) {
            $r['str_manage'] = '<a href="'.U(
                    "AdminTerm/add", array(
                                       "parent" => $r['term_id']
                                   )
                ).'">添加子类</a> | <a href="'.U(
                                   "AdminTerm/edit", array(
                                                       "id" => $r['term_id']
                                                   )
                               ).'">修改</a> | <a class="js-ajax-delete" href="'.U(
                                   "AdminTerm/delete", array(
                                                         "id" => $r['term_id']
                                                     )
                               ).'">删除</a> ';
            $r['visit'] = "<a href='#'>访问</a>";
    ...
    function delete() {
        ...
        if (isset($_POST['ids'])) {
            $app_ids = join(",", $_POST['ids']);
            $data['is_app'] = 3;
            if ($this->game_model->where("id in ($app_ids)")->save($data)) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
            }
    ...
    function check() {
        ...
        if (isset($_POST['ids']) && $_GET["online"]) {
            ...
            foreach ($ids as $v) {
                $id = $v;
                $packageurl = M('game_info')->where(array('app_id' => $id))->getField('yiosurl');
                if (empty($packageurl)) {
                    $this->success("请填写游戏下载地址", U('Sdk/Game/edit', array('id' => $id)));
                }
            }
            $app_ids = join(",", $_POST['ids']);
            if ($this->game_model->where("id in ($app_ids)")->save($data) !== false) {
