...
class Gift extends Base
{
    ...
    public function save() {
        ...
        // 查询礼包是否存在
        $gflog_info = DB::name('gift')->where(array(
            'id' => $giftid
        ))->find();
        if (empty($gflog_info)) {
    ...
    private function getGiftdetail($giftid) {
        ...
        $field = array(
            ...
            "FROM_UNIXTIME(gf.`start_time`, '%Y-%m-%d %H:%i')" => 'starttime',
            "FROM_UNIXTIME(gf.`end_time`, '%Y-%m-%d %H:%i')" => 'enttime',
            'gf.scope' => 'scope'
        );
        ...
        $data = DB::name('gift')->alias('gf')->field($field)->join($join)->where(
                array(
                    'gf.id' => $giftid
                ))->find();
        if (empty($data)) {
    ...
    private function setUserGift($giftid) {
        ...
        // 获取礼包信息
        $gift_info = DB::name('gift')->where(array(
            'id' => $giftid
        ))->find();
        ...
        if ($gift_info['remain'] > 0) {
            ...
            DB::name('gift')->update($gift_info);

            $gfc_model = DB::name('gift_code');
            // 查找未被领取的礼包
            ...
            if ($rs > 0) {
                ...
                DB::name('gift_log')->insert($gf_log);
    ...
    private function getUserGift($giftid, $mem_id) {
        ...
        $gflog_info = DB::name('gift_log')->where($giftmap)->find();
    ...
    private function getGiftlist(array $where = array()) {
        ...
        if (1 == $where['usergift']) {
            ...
            $count = DB::name('gift')->alias('gf')->where($map)->count();
            if ($count > 0) {
                $limit = $page . ',' . $offset;
                $data = DB::name('gift')->alias('gf')->field($field)->join($join)->where($map)->page($limit)->order(
                        'gf.id desc')->select();
                // 查询玩家是否已经领取过
                if ($this->mem_id > 0) {
                    foreach ($data as $k => $v) {
