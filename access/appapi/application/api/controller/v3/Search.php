...
class Search extends Base
{
    ...
    public function hotword() {
        $page = $this->request->get('page/d', 1); // 游戏ID.
        $offset = $this->request->get('offset/d', 10); // 游戏ID.
        $game_model = new GameModel();
        $data = $game_model->where(array('is_app'=>2))->page($page,$offset)->column('name');
        if (empty($data)) {
            return hs_api_responce('404', '无记录');
        }else{
            $rdata['count'] = count($data);
            $rdata['list'] = $data;
