...
function searchuser($username) {
    $members = M('members');
    $res = $members->where(array('username' => $username))->find();
    return $res;
}
...
function findMibao($username) {
    ...
    $data = $mibao->field($field)->where(array('mem_id' => $id))->find();
    return $data;
}
...
function paylist($username, $and) {
    ...
    $page->PageSize = 15;
    $mem_id = M('members')->where(array('username' => $username))->getField('id');
    $sql = "SELECT id,order_id,amount,ptb_cnt,create_time,status FROM c_ptb_pay WHERE mem_id='{$mem_id}' ".$and
           ." ORDER BY id DESC";
...
function xiaofeiList($username, $and) {
    ...
    $mem_id = M('members')->where(array('username' => $username))->getField('id');
    $sql
        = "SELECT id,order_id,amount,ptb_cnt,app_id,agent_id,create_time FROM c_ptb_pay WHERE mem_id='{$mem_id}' and status='2' AND "
          .$and." ORDER BY id DESC";
    $sql_total = "select count(*) as total FROM c_ptb_pay WHERE mem_id='{$mem_id}' and status='2' AND ".$and
                 ." ORDER BY id DESC";
    $data = M('ptbPay')->query($sql." limit  ".$page->OffSet());
...
function asklist($userid) {
    mysql_select_db("db_sdk_mn");
    $sql
        = "SELECT id,gameid,title,details,uid,contact,icon,FROM_UNIXTIME(reg_time) reg_time,status FROM c_myask WHERE uid='{$userid}' ORDER BY id DESC";
    $rs = mysql_query($sql);
    $data = array();
...
function checkask($userid, $id) {
    $dbh = new PDO('mysql:host='.C('DB_HOST').';dbname='.C('DB_NAME'), C('DB_USER'), C('DB_PWD'));
    $sql
        = "SELECT id,gameid,title,details,uid,contact,icon,FROM_UNIXTIME(reg_time) reg_time,status FROM c_myask
...
function gamelist() {
    $Game = M('game');
    $where['is_delete'] = 2;
    $data = $Game->where($where)->order('listorder ASC ,id ASC')->getfield(
        'id did, id,name,type,icon,create_time,status', true
    );
    return $data;
}
...
function hotgamelist() {
    $Game = M('game');
    $data = $Game->alias('g')->field('g.name,i.bigimage,i.url,i.publicity,i.url')->join(
        'c_game_info i on i.app_id = g.id'
    )->where("g.is_delete=2 and i.lanmu=2")->order('g.listorder DESC')->select();
    return $data;
}
...
function findgamelist($where) {
    ...
    $page->Total = $total;
    $data = $game_model->field('id,name,type,icon,create_time')->where($map)->where($where)->order(
        'listorder ASC ,id DESC'
    )->limit($page->Offset())->select();
    $showpage = $page->ShowLink();
    $arr = array();
    $arr['showpage'] = $showpage;
    $arr['list'] = $data;
...
function gameinfo($id) {
    ...
    $where['app_id'] = $id;
    $where['id_delete'] = 2;
    $data = $gameInfo->field('app_id,bigimage,smallimage,mobile_icon,url,iosurl,androidurl,description,function')
                     ->where($where)->select();
    return $data;
}
...
function newslist($where) {
    ...
    $page->Total = $count;
    $newlist = $webArticle->field('id,title,image,create_time,total,is_top')->where($map)->where($where)->order(
...
function findnews($where) {
    ...
    $data = $webArticle->where($where)->order("is_top,create_time DESC")->select();
    return $data;
}
...
function serverlist() {
    ...
    $where['ws.is_delete'] = 2;
    $where['ws.start_time'] = array('lt', time());
    $data = $webServer
        ->alias('ws')
        ->field('ws.app_id,ws.sername,ws.start_time,ws.status,ws.gift_url,g.mobile_icon image')
        ->join('left join '.C("DB_PREFIX").'game g on g.id=ws.app_id')
        ->where($where)
        ->order('ws.start_time desc')->select();
    $whereb['ws.is_delete'] = 2;
    $whereb['ws.start_time'] = array('gt', time());
    $datab = $webServer
        ->alias('ws')
        ->field('ws.app_id,ws.sername,ws.start_time,ws.status,ws.gift_url,g.mobile_icon image')
        ->join('left join '.C("DB_PREFIX").'game g on g.id=ws.app_id')
...
function getMedia() {
    $webMedia = M('webMedia');
    $map['name'] = array("neq", "agent_site_pics");
    $data = $webMedia->where($map)->select();
    return $data;
}
...
function strcode($mem_id, $gf_id) {
    ...
    $data = $gift->alias('g')->field('gc.code')->join('LEFT JOIN c_gift_code gc on g.id=gc.gf_id')->where($where)
                 ->order('g.start_time ASC')->find();
    return $data;
}
...
function setlibao($mem_id, $gf_id, $code) {
    $giftCode = M("giftCode");
    $id = $giftCode->where(array('code' => $code))->getField('id');
    $map['mem_id'] = $mem_id;
    $map['update_time'] = time();
    $rs = $giftCode->where(array('id' => $id))->save($map);  //领取礼包
    $grs = M('gift')->where(array('id' => $gf_id))->setDec('remain');   //领取之后礼包留存数减一
    if ($rs && $grs) {
        $map['gf_id'] = $gf_id;
