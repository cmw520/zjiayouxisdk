...
class Game extends Base
{
    ...
    public function index() {
        ...
        $map['from'] = $this->request->get('from/d', 0); // 每页请求数量，默认为10
...
/*        $field = array(
            ...
            "IFNULL(gi.description,'')" => 'disc',
            'IFNULL(ge.star_cnt,10)' => 'score',
            "FROM_UNIXTIME(g.run_time, '%Y-%m-%d')" => 'runtime',
        ...
        //test
        $game_info['newscnt'] = $this->getGiftcntbyGameid($map['id']);
        $game_info['distype'] = $this->getGiftcntbyGameid($map['id']);
        $game_info['rebate'] = $this->getGiftcntbyGameid($map['id']);


        if ($this->agent_id>0){
            //查询此agent_id的rebate_rate
            $agent_rebate = $this->getAgentrebate($this->agent_id,$game_info['id']);
            if (!empty($agent_rebate)){
                $data[$k]['distype'] = 2;
                $data[$k]['rebate'] = $agent_rebate;
            }
        }
    ...
    // 后去新闻数量
    public function getNewscntbyGameid($app_id = 0) {
        $app_id = (int) $app_id;
        if ($app_id <= 0) {
            return 0;
        }
        $map['app_id'] = $app_id;
        $map['post_status'] = 2;
        return DB::name('posts')->where($map)->count();
    }
    ...
    private function getGameList(array $where = array()) {
        ...
        $map['g.classify'] = $where['from']; // 游戏上线才显示
        $data = array();
        $page = $where['page'];
        $offset = 10;
...
//         $field .= ",gi.androidurl downlink,gi.publicity oneword,gi.size ";
        ...
        $field = array(
            ...
            'g.category' => 'category',
            'IFNULL(ge.down_cnt,0)' => 'downcnt',
            'IFNULL(gr.benefit_type,0)' => 'distype',
            'IFNULL(gr.mem_rate,1)' => 'discount',
            'IFNULL(gr.mem_rebate,0)' => 'rebate',
            "IFNULL(gi.androidurl,'')" => 'downlink',
            "IFNULL(gi.publicity,'')" => 'oneword',
            'IFNULL(ge.star_cnt,10)' => 'score',
            'IFNULL(ge.like_cnt,10)' => 'likecnt',
            'IFNULL(ge.share_cnt,10)' => 'sharecnt',
            "FROM_UNIXTIME(g.run_time, '%Y-%m-%d')" => 'runtime',
            'g.packagename' => 'packagename'
        );
        ...
        $count = Db::name('game')->alias('g')->join($join)->where($map)->count();
        ...
        if ($count > 0) {
            ...
            $limit = $m . ',' . $offset;
            $data = Db::name('game')->alias('g')->field($field)->join($join)->where($map)->limit($limit)->select();
            $count = count($data);
        }

        foreach ($data as $k => $v){
            $data[$k]['giftcnt'] = $this->getGiftcntbyGameid($v['gameid']);
//             $data[$k]['newscnt'] = $this->getGiftcntbyGameid($v['id']);
            if ($this->agent_id>0){
                //查询此agent_id的rebate_rate
                $agent_rebate = $this->getAgentrebate($this->agent_id,$v['id']);
                if (!empty($agent_rebate)){
                    $data[$k]['distype'] = 2;
                    $data[$k]['rebate'] = $agent_rebate;
                }
    ...
    private function getAgentrebate($agent_id,$gameid){
        if ($agent_id<=0){
            return 0;
        }
    ...
    private function getRecList(array $where = array()){
        $rdata = array();
        $map['g.is_app'] = 2; // app中上线的游戏
        $map['g.is_delete'] = 2; // 伪删除游戏不显示
        $data = array();
        ...
        if ($count > 0) {
            ...
            $data = $gr_model->alias('gr')->field($field)->join($join)->where($map)->order($order)->limit($limit)->select();
            $count = count($data);
        }
