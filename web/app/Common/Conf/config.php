...
if (file_exists(SITE_PATH."conf/pay/alipay/config.php")) {
    $alipay_config = include SITE_PATH."conf/pay/alipay/config.php";
} else {
    $alipay_config = array();
}
if (file_exists(SITE_PATH."conf/web/config.php")) {
    $runtime_config = include SITE_PATH."conf/web/config.php";
} else {
    $runtime_config = array();
}
if (file_exists(SITE_PATH."conf/web/route.php")) {
    $routes = include SITE_PATH.'conf/web/route.php';
} else {
    $routes = array();
}
if (file_exists(SITE_PATH."conf/web/uc_config.inc.php")) {
    $uc_config = include SITE_PATH.'conf/web/uc_config.inc.php';
} else {
    $uc_config = array();
}
$configs = array(
    ...
    'MODULE_ALLOW_LIST'            => array('Web', 'Admin', 'Portal', 'Asset', 'Api', 'User', 'Wx', 'Comment', 'Qiushi',
                                            'Tpl',
                                            'Topic', 'Install', 'Bug', 'Better', 'Pay'),
    ...
    'SP_TMPL_PATH'                 => SITE_PATH.'web/themes/',       // 前台模板文件根目录
    ...
    'SP_ADMIN_STYLE'               => 'flat',
    'SP_ADMIN_TMPL_PATH'           => SITE_PATH.'web/themes/',       // 各个项目后台模板文件根目录
...
return array_merge($configs, $db, $runtime_config, $alipay_config);
